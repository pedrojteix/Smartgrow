...
// Reles -- SSR 4 canais, ativo LOW
#define RELE_LUZ      18   // IN1 -- luz (SSR AC)
// CH3 e CH4 do rele queimaram (contato soldado). Apenas CH1 e CH2 funcionam.
// SOLUCAO TEMPORARIA: as duas bombas no MESMO canal CH2 (GPIO23) -- acionadas juntas
#define RELE_BOMBAS   23   // IN2 (CH2) -- aciona AS DUAS bombas simultaneamente
#define RELE_ON       LOW
...
void setup() {
  ...
  exibirBoot();
  Serial.println(F("[SGRW] R07 iniciado -- FatorK=173.92 | Transplante 30/04/2026"));
}
...
void iniciarRega(float volumeBase_mL, int numBomba) {
  ...
  volumeAlvo_mL   = volumeBase_mL * 2.0;
  volumeAtual_mL  = 0.0;

  // Timeout por tempo calibrado (mais confiavel que pulsos)
  float tempoCalculado = (volumeBase_mL * 2.0) / VAZAO_ML_S;
...
void exibirBoot() {
  display.clearDisplay();
  display.setTextSize(1);
  display.setCursor(20, 20); display.print("SmartGrow R07");
...
void enviarParaNuvem(bool manual) {
  ...
  Serial.printf("[NUVEM] Payload size: %d bytes\n", payload.length());
  Serial.printf("[NUVEM] Heap antes: %d\n", ESP.getFreeHeap());
  ...
  Serial.printf("[NUVEM] POST code: %d\n", code);

  if (code == 302 || code == 301) {
    String loc = http.getLocation();
    Serial.println("[NUVEM] Redirect para: " + loc.substring(0, 80));
    http.end();
    ...
    HTTPClient http2;
    http2.begin(client2, loc);
    http2.setTimeout(15000);
    int code2 = http2.GET();
    Serial.printf("[NUVEM] GET code2: %d\n", code2);
    if (code2 == 200) {
      String resp = http2.getString();
      Serial.println("[NUVEM] Resp: " + resp.substring(0, 100));
    }
    http2.end();
  }
  http.end();
  ...
  Serial.printf("[NUVEM] Heap livre: %d bytes\n", ESP.getFreeHeap());
  ...
  if (code1 == 302 || code1 == 301) {
    ...
    if (code2 == 200) {
      ...
      if (!deserializeJson(respDoc, resp)) {
        ...
        // IA retorna volume por vaso -- bomba 1 rega vaso 1, bomba 2 rega vaso 2
        // Por ora aciona ambas em sequencia: 1 depois 2
        // TODO: implementar fila de rega para acionar as duas em sequencia
        if (regarAgora && volumeBaseIA_mL > 0) iniciarRega(volumeBaseIA_mL, 1);
        // Verifica comando de rega remota do site
        ...
        // Lê bloqueio remoto da bomba via site
        int bloqueio = respDoc["bomba_bloqueada"] | -1;
        if (bloqueio == 1) {
          bombaRemotaBloqueada = true;
          Serial.println(F("[REMOTO] Bomba BLOQUEADA pelo site"));
          enviarEventoParaNuvem("bomba_bloqueada", "Bomba bloqueada remotamente pelo site");
        } else if (bloqueio == 0) {
          bombaRemotaBloqueada = false;
          Serial.println(F("[REMOTO] Bomba DESBLOQUEADA pelo site"));
          enviarEventoParaNuvem("bomba_bloqueada", "Bomba desbloqueada remotamente pelo site");
        }
        // Se bloqueio == -1 (campo ausente na resposta), nao muda o estado atual
      } else {
