...
// + Deteccao de sensor falho (queda >30% em 1 leitura)
// + Rega remota via site (polling GAS a cada ciclo)
// + Rega manual BOOT reescrita (case 5 robusto)
...
// ?? Deteccao de sensor falho ??????????????????????????????????
// Se um sensor cair >30% em uma unica leitura (2s), entra em
// estado SUSPEITO. Apos 3 leituras suspeitas consecutivas vira
// FALHO e e ignorado -- o outro sensor assume para todo o sistema.
// Subidas rapidas (pos-rega) NAO disparam suspeita.
int   ultimoSolo1Pct       = -1;   // valor anterior para comparar
int   ultimoSolo2Pct       = -1;
int   suspeito1Contagem    = 0;    // leituras suspeitas consecutivas
int   suspeito2Contagem    = 0;
bool  sensor1Falho         = false;
bool  sensor2Falho         = false;
const int LIMIAR_QUEDA     = 30;   // queda maxima permitida por leitura (%)
const int LEITURAS_FALHO   = 3;    // leituras suspeitas para declarar falho
...
bool          regaManualAtiva   = false;  // rega manual via BOOT (case 5)
unsigned long tRegaManualInicio = 0;      // inicio da rega manual
const unsigned long REGA_MANUAL_MAX_MS = 60000; // timeout: 1 minuto

// ?? Rega remota via site ??????????????????????????????????????
// O site envia comando ao GAS. O ESP32 consulta o GAS a cada
// ciclo de envio (15min) e executa se houver comando pendente.
float regaRemotaVolume_mL  = 0.0;  // 0 = sem comando pendente
...
// ============================================================
// 14b. FORWARD DECLARATIONS
// Necessario porque loop() chama funcoes definidas depois dele
// ============================================================
...
void setup() {
  ...
  exibirBoot();
  Serial.println(F("[SGRW] R06 iniciado -- FatorK=173.92 | Transplante 30/04/2026"));
}
...
void loop() {
  ...
  if (executandoRega) {
    ...
    // Rega manual BOOT: para pelo timeout ou clique -- nao pelo volume
    if (regaManualAtiva) {
      if (millis() - tRegaManualInicio >= REGA_MANUAL_MAX_MS) {
        pararBomba("Rega manual: timeout 60s");
        regaManualAtiva = false;
        exibirFeedback("Rega manual", "encerrada 60s");
      }
      // parada por clique e tratada em processarBotao()
    } else {
      // Rega normal (IA ou remota): para pelo volume
...
void lerSensores() {
  ...
  // Aplica apenas para QUEDAS -- subidas rapidas (pos-rega) sao normais
  if (ultimoSolo1Pct >= 0 && !sensor1Falho) {
    int queda1 = ultimoSolo1Pct - lido1; // positivo = queda
    if (queda1 >= LIMIAR_QUEDA) {
      suspeito1Contagem++;
      Serial.printf("[SENS] S1 suspeito: %d->%d (queda %d%%) [%d/%d]\n",
                    ultimoSolo1Pct, lido1, queda1, suspeito1Contagem, LEITURAS_FALHO);
      if (suspeito1Contagem >= LEITURAS_FALHO) {
        sensor1Falho = true;
        Serial.println(F("[SENS] S1 declarado FALHO -- usando S2 para ambos os vasos"));
        enviarEventoParaNuvem("sensor_falho", "S1 falhou: queda abrupta detectada");
      }
    } else {
      suspeito1Contagem = 0; // leitura normal, reseta suspeita
    }
  }
  if (ultimoSolo2Pct >= 0 && !sensor2Falho) {
    int queda2 = ultimoSolo2Pct - lido2;
    if (queda2 >= LIMIAR_QUEDA) {
      suspeito2Contagem++;
      Serial.printf("[SENS] S2 suspeito: %d->%d (queda %d%%) [%d/%d]\n",
                    ultimoSolo2Pct, lido2, queda2, suspeito2Contagem, LEITURAS_FALHO);
      if (suspeito2Contagem >= LEITURAS_FALHO) {
        sensor2Falho = true;
        Serial.println(F("[SENS] S2 declarado FALHO -- usando S1 para ambos os vasos"));
        enviarEventoParaNuvem("sensor_falho", "S2 falhou: queda abrupta detectada");
      }
    } else {
      suspeito2Contagem = 0;
    }
  ...
  // ?? Aplica valores: se um sensor estiver falho, usa o outro ??
  if (!sensor1Falho && !sensor2Falho) {
    umidadeSolo1Pct = lido1;
    umidadeSolo2Pct = lido2;
  } else if (sensor1Falho && !sensor2Falho) {
    umidadeSolo1Pct = lido2; // S1 falho: usa S2 para ambos
    umidadeSolo2Pct = lido2;
  } else if (!sensor1Falho && sensor2Falho) {
    umidadeSolo1Pct = lido1;
    umidadeSolo2Pct = lido1; // S2 falho: usa S1 para ambos
  } else {
    // Ambos falhos: usa ultima leitura conhecida (nao atualiza)
    Serial.println(F("[SENS] AMBOS os sensores falhos -- mantendo ultima leitura"));
  }

  // Guarda leituras brutas para comparacao na proxima iteracao
  // (usa lido1/lido2, nao os ajustados, para detectar quedas reais)
...
void exibirPaginaSensores() {
  ...
  // Solo 1 e Solo 2 -- mostra FALHO se sensor fora
...
void exibirBoot() {
  display.clearDisplay();
  display.setTextSize(1);
  display.setCursor(20, 20); display.print("SmartGrow R06");
...
void processarBotao() {
  ...
  if (!botaoSegurando && !gestoConcluido && contadorCliques > 0
    ...
    // Durante rega manual: 1 clique para a bomba e volta ao normal
    if (regaManualAtiva && contadorCliques == 1) {
      regaManualAtiva = false;
      pararBomba("Rega manual: parada pelo usuario");
      exibirFeedback("Rega manual", "parada");
      Serial.println(F("[BTN] Rega manual parada pelo usuario"));
...
void executarGesto(int cliques) {
  ...
  switch (cliques) {
    ...
    case 5:
      // Rega manual: bomba liga por ate 60s ou ate 1 clique simples
      // Usa flag regaManualAtiva -- NAO usa volumeAlvo, NAO interfere
      // com o loop de volume normal.
      if (nivelBaixo) { exibirFeedback("Reserv.", "vazio!"); break; }
      if (executandoRega) { exibirFeedback("Ja regando", "aguarde"); break; }
      // Inicializa estado da rega manual
      regaManualAtiva   = true;
      tRegaManualInicio = millis();
      // Liga bomba diretamente sem passar por iniciarRega()
      // (que calcularia volume alvo e pararia pelo fluxo)
      noInterrupts(); pulsosFluxo = 0; interrupts();
      volumeAtual_mL  = 0.0;
      volumeAlvo_mL   = 999999.0; // nunca para por volume
      executandoRega  = true;
      digitalWrite(RELE_BOMBA, RELE_ON);
      tInicioRega_ms  = millis();
      // Evento de inicio
      if (WiFi.status() == WL_CONNECTED) {
        int h=0,m=0,s=0; struct tm ti;
        if (getLocalTime(&ti)){h=ti.tm_hour;m=ti.tm_min;s=ti.tm_sec;}
        char desc[64];
        snprintf(desc,sizeof(desc),"Bomba ON %02d:%02d:%02d -- rega manual BOOT (max 60s)",h,m,s);
        enviarEventoParaNuvem("bomba_on", desc);
      }
      exibirFeedback("Rega manual", "clique=stop");
      Serial.println(F("[BTN] Rega manual iniciada -- max 60s, 1 clique para"));
      break;
...
void enviarParaNuvem(bool manual) {
  ...
  doc["sensor1Falho"]     = sensor1Falho ? 1 : 0;
  doc["sensor2Falho"]     = sensor2Falho ? 1 : 0;
  ...
  if (code1 == 302 || code1 == 301) {
    ...
    if (code2 == 200) {
      ...
      if (!deserializeJson(respDoc, resp)) {
        ...
        // Verifica se o site enviou comando de rega remota
        ...
        if (volRemoto > 0 && volRemoto <= GALAO_TOTAL_ML) {
          ...
          Serial.printf("[REMOTO] Comando de rega remota recebido: %.0fmL\n", volRemoto);
        }
      } else {
