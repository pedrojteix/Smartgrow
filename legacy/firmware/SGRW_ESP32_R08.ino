...
void setup() {
  ...
  exibirBoot();
  Serial.println(F("[SGRW] R08 iniciado -- FatorK=173.92 | Transplante 30/04/2026"));
}
...
void exibirBoot() {
  display.clearDisplay();
  display.setTextSize(1);
  display.setCursor(20, 20); display.print("SmartGrow R08");
...
void enviarParaNuvem(bool manual) {
  ...
  // Rega automatica decidida pela IA
  if (regarAgora && volumeBaseIA_mL > 0) {
    strcpy(regaTrigger, "auto");
    iniciarRega(volumeBaseIA_mL, 1);
  }
